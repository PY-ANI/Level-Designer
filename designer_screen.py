...
class animated_tile():
    ...
    def draw(self,win,sc_x,sc_y,current_frame):
        win.blit(pygame.transform.scale(pygame.image.load(self.frames[int(current_frame*self.frame_count)%self.frame_count]).convert_alpha(),(self.size,self.size)),(self.rect.x+sc_x,self.rect.y+sc_y))
    ...
    def get_sprite(self,pos_x:int,pos_y:int,surf_path:str|bytes|list,current_dir_path:str|bytes):
        ...
        if k_y in self.sprites[self.layer].keys():
            if surf_path == 'p' or surf_path == 'e':
                ...
                self.sprites[self.layer][k_y][k_x] = animated_tile((pos_x-pos_x%self.sprite_size),(pos_y-pos_y%self.sprite_size),self.sprite_size,surf_path)
